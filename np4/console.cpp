...
void splitAndParse(string str){
	...
	while(getline(delim,token,'&')){
		...
		if(count<=4){
			...
			sh = token.substr(token.find('=')+1);
			count++;
		}else{
			sp = token.substr(token.find('=')+1);
		}
	}
}
...
struct Client : public enable_shared_from_this<Client> {
  Client(string session, string file_name, tcp::resolver::query q , string sh , string sp)
      : session(move(session)), q(move(q)),q_connection{sh,sp}  {
  ...
  void connect_handler() {
	auto http_endpoint = *resolver.resolve(q);
	socks4::request socks_request(socks4::request::connect, http_endpoint, "");
	boost::asio::write(tcp_socket, socks_request.buffers());
	socks4::reply socks_reply;
    boost::asio::read(tcp_socket, socks_reply.buffers());	
  ...
  void resolve() {
    auto self(shared_from_this());
    resolver.async_resolve(q_connection,[this, self](const boost::system::error_code &ec,tcp::resolver::iterator it) {
                             if (!ec)
                               resolve_handler(it);
                           });
  }
  ...
  tcp::resolver::query q_connection;
...
int main(){
	...
	for (int i = 0; i < 5; ++i) {
	  if(Host[i].name!=""){
		...
		make_shared<Client>("s" + to_string(i), Host[i].file, move(q) , sh, sp)
          ->resolve();
	  }
	}
